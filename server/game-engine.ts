...
export interface PetState {
  ...
  mood: 'happy' | 'sad' | 'sleepy' | 'excited' | 'hungry' | 'neutral' | 'grumpy';
...
export function processAction(pet: PetState, action: ActionName): PetState {
  ...
  for (const item of newUnlocks) {
    if (['toy-ball', 'crown'].includes(item) && !next.inventory.includes(item)) {
  ...
  next.interactionHistory.push({
    action,
    timestamp: Date.now(),
    statsAfter: { hunger: next.hunger, happiness: next.happiness, energy: next.energy, affection: next.affection },
  });
...
function checkUnlocks(pet: PetState): string[] {
  ...
  if (pet.affection >= 80 && !pet.unlockedComponents.includes('hug')) unlocks.push('hug');
  if (pet.happiness >= 60 && !pet.unlockedComponents.includes('toy-ball')) unlocks.push('toy-ball');
  if (pet.affection >= 90 && !pet.unlockedComponents.includes('crown')) unlocks.push('crown');
  if (pet.totalInteractions >= 20 && !pet.unlockedComponents.includes('golden-room')) unlocks.push('golden-room');
  if (pet.happiness >= 90 && pet.energy >= 60 && !pet.unlockedComponents.includes('dance')) unlocks.push('dance');
  return unlocks;
}

function generatePosition(mood: PetState['mood'], location: PetState['location']): { x: number; y: number } {
...
export function generateThought(pet: PetState, action: ActionName): string {
  const thoughts: Record<string, Record<string, string[]>> = {
    feed: {
      happy: ['Yum! That was delicious!', 'More please! I love this!', 'So tasty! Thank you!'],
